...
namespace FriendSystem
{
    ...
    public class GoodFriendManager : MonoBehaviour
    {
        ...
        public ChatPage chatWindow;                                                           // 聊天窗口
        #region 属性
        ...
        public void OnTagInit()
        {
            InfoHandle.GetMyTags(json =>
            {
                ...
                if (code == 200)
                {
                    ...
                    {
                        if (!myTags.Any(tag => tag.name == item.name))
                        {
                            myTags.Add(item);
                        }
                    }
        ...
        private void InitFriendTag()    // 初始化好友标签
        {
            foreach (var tag in myTags)
            {
                InfoHandle.GetStudentsByTag(tag.labelId, json =>
                 {
                     JObject obj = (JObject)JsonConvert.DeserializeObject(json);
                     string msg = (string)obj["msg"];
                     int code = (int)obj["code"];
        ...
        public void OnChatMsg(string json)
        {
            ...
            if (Global.chatNo == receiveChatNo)
            {
                if (friendList.ContainsKey(sendChatNo))
                {
                    ...
                    friendList[sendChatNo].unReadCount++;
                    if (chatWindow?.target?.friendChatNo == sendChatNo && chatWindow.gameObject.activeInHierarchy)
                    {
