...
namespace Invector.vCharacterController
{
    public class vThirdPersonMotor : MonoBehaviour
    {
        ...
        void NewAnimator()
        {
            ...
            if ((Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.W) || Input.GetKey(KeyCode.S) || Input.GetKey(KeyCode.D)) && Input.GetKey(KeyCode.LeftShift))
            {
                animator.Play("Run");
                if(TryGetComponent(out PlayerStatusView view))
                {
                    view.m_AnimatotName = "Run";
                }
                //GetComponent<SyncObject>().currentPlayerState = "Run";
                //GameManager.GetInstance().currentPlayerState = EnumManager.PlayerState.Run;
            ...
            else if (Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.W) || Input.GetKey(KeyCode.S) || Input.GetKey(KeyCode.D))
            {
                ...
                if (TryGetComponent(out PlayerStatusView view))
                {
                    view.m_AnimatotName = "Walk";
                }                //GameManager.GetInstance().currentPlayerState = EnumManager.PlayerState.Walk;

            }
            else if (Input.GetMouseButton(1) && Player.isSeatDown == false)
            {
                if (TryGetComponent(out PlayerStatusView view))
                {
                    view.m_AnimatotName = "Idle";
                }
                //GetComponent<SyncObject>().currentPlayerState = "Idle";
                animator.Play("Idle");
            }
            if (!Input.anyKey&& Player.isSeatDown == false)
            {
                animator.Play("Idle");
                if (TryGetComponent(out PlayerStatusView view))
                {
                ...
                //GameManager.GetInstance().currentPlayerState = EnumManager.PlayerState.Idle;
                //isAnimator = true;
            }
        ...
        public void RequestSeat(Transform seat, int seatNum)// 请求座位
        {
            if (Player.isSeatDown) return;
            seatTransform = seat;
        ...
        {
            if (Player.isSeatDown) return;
            Player.isSeatDown = true;
            animator.Play("SitDown");
          
            transform.GetComponent<Rigidbody>().isKinematic = true;
            //transform.GetComponent<Rigidbody>().MovePosition(seat.position - Vector3.up * 1);
            transform.position = seatTransform.position - Vector3.up * 0.8f;
            transform.forward = seatTransform.right * -1;
        }
        public void SittingApart() // 离开座位
        {
            //transform.GetComponent<Rigidbody>().MovePosition(transform.position + Vector3.up * 1);
            transform.position = transform.position + Vector3.up * 1;
            transform.GetComponent<Rigidbody>().isKinematic = false;
            Player.isSeatDown = false;
        }
        #endregion
