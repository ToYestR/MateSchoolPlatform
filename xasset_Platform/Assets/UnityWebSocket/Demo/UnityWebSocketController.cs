...
public class UnityWebSocketController : MonoBehaviour
{
    public static UnityWebSocketController Instance;

    public string address = "ws://ijob-x.com/ws?appid=100001";
    ...
    private void Socket_OnOpen(object sender, OpenEventArgs e)
    {
        ...
        jobect.Add("action", "10001");
        jobect.Add("sendChatNo", Global.chatNo);
        print(jobect.ToString());
        Send(jobect.ToString());
    }
    ...
    /// 发送聊天信息
    /// </summary>
    /// <param name="ToChatNo"></param>
    /// <param name="content"></param>
    public void SendChatMessage(string ToChatNo,string content)
    {
        JObject jobect = new JObject();
        jobect.Add("action", "10010");
        jobect.Add("chatType", "1");
        jobect.Add("msgType", "1");
        jobect.Add("sendChatNo", Global.chatNo);
        jobect.Add("sendNickName",Global.nickname);
        jobect.Add("receiveChatNo", ToChatNo);
        jobect.Add("content", content);
        socket.SendAsync(jobect.ToString());
    }
    private void Socket_OnMessage(object sender, MessageEventArgs e)
    {
        ...
        //JObject data = JsonConvert.DeserializeObject<JObject>(jobject["data"].ToString());
        //if (data.ContainsKey("action"))
        //{
        //    if (data["action"].ToString() == "10010")
        //    {
        //        OnChatMessage.Invoke(data.ToString());
        //    }
        //}
        //if (e.IsBinary)
        //{
        ...
        print("收到服务器消息：\n" + e.Data);
        JObject jobject = JsonConvert.DeserializeObject<JObject>(e.Data);
        string jsonData = jobject["data"].ToString();
        switch (jobject["state"].ToString())
        {
            case "SYS_PUSH":
                FriendSystem.GoodFriendManager.getInstance?.OnChatOnLineChange(jsonData);
                break;
            case "CHAT":
                FriendSystem.GoodFriendManager.getInstance?.OnChatMsg(jsonData);
                break;
            case "ACK":
                break;
            default:
